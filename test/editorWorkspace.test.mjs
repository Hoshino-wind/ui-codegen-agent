...
test("createEditorWorkspace derives preview, export, and verifier output from one LayerDoc", () => {
  ...
  assert.equal(workspace.projectExport.manifest.assetIndexSchema, "asset-index.schema.json");
  ...
  assert.equal(workspace.projectExport.files.length, 39);
  ...
  assert.equal(workspace.projectExport.files.some((file) => file.path === "asset-index.schema.json"), true);
