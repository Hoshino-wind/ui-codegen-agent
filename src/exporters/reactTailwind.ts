import type { AssetNode, LayerDoc, LayerNode, Rect, SectionNode } from "../layerdoc/types.js";
...
function inlineStyle(bounds: Rect): string {
...
function renderLayer(doc: LayerDoc, layer: LayerNode): string {
  const baseProps = `data-layer-id="${escapeAttribute(layer.id)}" data-kind="${layer.kind}" data-track="${layer.track}" className="absolute" style=${inlineStyle(layer.bounds)}`;
